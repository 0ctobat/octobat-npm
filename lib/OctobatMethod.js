 ...
 */
module.exports = function octobatMethod(spec) {
  ...
  return function() {
    ...
    const { promise, resolve, reject } = this.createDeferred(callback);
    ...
    for (var i = 0, l = urlParams.length; i < l; ++i) {
      ...
      if (!arg) {
        ...
        throw new Error('Octobat: I require argument "' + urlParams[i] + '", but I got: ' + arg);
      }
    ...
    if (args.length) {
      throw new Error(
        'Octobat: Unknown arguments (' + args + '). Did you mean to pass an options object? ' +
        'See https://github.com/octobat/octobat-node/wiki/Passing-Options.'
      );
    }
