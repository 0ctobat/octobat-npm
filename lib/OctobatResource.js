...
OctobatResource.prototype = {
        ...
        try {
          ...
          if (response.error) {
            ...
            if (res.statusCode === 401) {
              err = new Error.OctobatAuthenticationError(response.error);
            } else {
              err = Error.OctobatError.generate(response.error);
            }
  ...
  _request: function(method, path, data, auth, options, callback) {
    ...
    var headers = {
      ...
      'User-Agent': 'Octobat/v1 NodeBindings/' + this._octobat.getConstant('PACKAGE_VERSION')
    };
